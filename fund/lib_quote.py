...
class LibQuote(MyLogger):
    ...
    def quote_trade_info(self, code: list):  # turnover
        ...
        for i in range(len(code)):
            ...
            #  3.匹配数据
            if code[i][0:3] in '688':
                results = self.__quote_match_kcb(text)
            else:
                results = self.__quote_match_zhuban(text)
            #  4.转储数据
            la.append(results)
    ...
    def __quote_match_zhuban(self, content):
        """
        主板:基本股票交易信息匹配规则
        :param content:网页内容
        :return:
    ...
    def __quote_match_kcb(self, content):
        """
        科创板: 基本股票交易信息匹配规则
        :param content:网页内容
        :return:
    ...
    def save_to_excel(self, df_data, filename: str, index=False):
        """
        保存数据到xlsx文件
        :param filename: 保存文件名
        :param df_data: dataframe格式数据
        :param index: 是否显示索引,默认不
        :return:
        """
        assert filename, "文件名不能为空"

        df_data.to_excel(filename, index=index)
...
if __name__ == "__main__":
    ...
    # quo.save_to_excel(zs_info,'test1.xlsx')
